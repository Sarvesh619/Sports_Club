...
<?php
        ...
        if(isset($_POST['addtourn'])){
          include('connection.php');
           $sdate=$_POST['sdate'];
            $edate=$_POST['edate'];
            $tname=$_POST['tname'];
            $sid=$_POST['sport'];
              $query5="select * from tour where (tname='$tname' AND sid='$sid' AND sdate='$sdate' AND edate='$edate')";
              $query_run5 = mysqli_query($connection,$query5);
              if(mysqli_num_rows($query_run5)>0){
              echo "<script type='text/javascript'>
              alert('Duplicate tournament occur cross checked it...');
              window.location.href = 'dashboard.php';  
            </script>";
            }
        else{
          $query = "insert into tour values(null,'$_POST[tname]','$_POST[sport]','$_POST[sdate]','$_POST[edate]')";
            ...
            if($query_run){
              ...
              echo "<script type='text/javascript'>
                  ...
                  window.location.href = 'AddTournament.php';
              </script>";
            }
          }
        }
...
<body>
  <div class="container">
    <div class="title">Add Tournament</div>
    <div class="content">
      <form action=" " method="post" name="frm" onsubmit="return validateform()">
        <div class="user-details">
          ...
          <div class="input-box">
            ...
            <input type="text" placeholder="Enter Tournament Name" name="tname" required>
          </div>
          <div class="input-box">
            <span class="details">Start Date</span>
            <input type="date" name="sdate" id="StartDate" min=<?php echo $dt;?> placeholder="Start Date" required>
          </div><br>
          <div class="input-box">
            <span class="details">End Date</span>
            <input type="date" class="form-control" name="edate" min="sdate" id="EndDate"  placeholder="End Date"  required>
          </div>
        <div class="button"><br><br>
          <input type="submit"  name="addtourn"  value="Add Tournament">
        </div>
      </form>
    </div>
  </div>
...
var start = document.frm.StartDate.value;
var end = document.frm.EndDate.value;
