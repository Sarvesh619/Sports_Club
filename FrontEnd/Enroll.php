...
$dt = date('Y-m-d');
$cid=$_GET['cid'];
$query1="select fee from course where cid='$cid'";
$query1_run=mysqli_query($connection,$query1);
while($row4 = mysqli_fetch_assoc($query1_run))
      { $fee=$row4['fee'];
...
$query3="select * from enrollc where (user='$uname' AND cid= '$cid' AND fee='$fee' AND enrolldate='$dt')";
...
$query2 = "insert into enrollc values(null,'$_SESSION[uname]','$_GET[cid]','$fee','$dt','$serializepmt')";
...
echo mysqli_error($connection);
	if(mysqli_affected_rows($connection) == 1)
	{
		...
		echo "<script>window.location='payment_receipt1.php?fee_payment_id=$insid';</script>";
	}
}
} 
      }
			...
			<div class="col-md-10 col-sm-offset-1 book-appointment" >
				...
				<form action="" method="post" name="frm" onsubmit="return validateform()">
					<div class="col-md-6 gaps">
						...
						<input type="number" style="background: rgb(255 219 219);" readonly name="balance"  value="<?php echo $fee; ?>" />
					</div>
