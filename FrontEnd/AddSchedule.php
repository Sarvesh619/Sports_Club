...
<?php
        ...
        $query2="select * from sch";
        $query_run2 = mysqli_query($connection,$query2);
        while($row1 = mysqli_fetch_assoc($query_run2))
          { $time=$_POST['time'];
            if($time==$row1['time']){
              echo "<script type='text/javascript'>
              alert('Duplicate time are not allowed../');
              window.location.href = 'AddSchedule.php';  
            </script>";
            }
          }
    
          $query = "insert into sch values(null,'$_POST[sport]','$_POST[time]','$_POST[day]')";
      ...
      <form action=" " method="post">
        <div class="user-details">
          ...
          <div class="input-box">
            ...
            <input type="time" class="form-control" id="time" name="time" placeholder="Sport time" pattern="([01]?[0-9]|2[0-3]):[0-5][0-9]" title="Enter Correct format" required>
          </div>
          <div class="input-box">
            <span class="details">Week Day</span>
        ...
        <div class="button"><br><br>
          <input type="submit"  name="addsch"  value="Add Schedule">
        </div>
      </form>
    </div>
  </div>
