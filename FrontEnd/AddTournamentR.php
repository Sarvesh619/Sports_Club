...
<?php
        ...
        $tid=$_GET['tid'];
         $sql = "SELECT tname,sdate,edate from tour where tid=$tid";
        $query1= mysqli_query ($connection, $sql);
        if(isset($_POST['addtourres'])){
          include('connection.php');
       $matchno=$_POST['matchno'];
        $tid=$_GET['tid'];
        $matchdes=$_POST['matchdes'];
        $date=$_POST['date'];
        $Result=$_POST['Result'];
          $query5="select * from tourres where (tid='$tid' AND matchno='$matchno' AND matchdes='$matchdes' AND date='$date' AND Result='$Result')";
              $query_run5 = mysqli_query($connection,$query5);
              if(mysqli_num_rows($query_run5)>0){
          echo "<script type='text/javascript'>
          alert('Duplicate Tournament Result ....');
          window.location.href = 'SelectT.php';  
        </script>";
        }
    else{
    $query = "insert into tourres values(null,'$_GET[tid]','$_POST[matchno]','$_POST[date]','$_POST[matchdes]','$_POST[Result]')";
...
<html lang="en" dir="ltr">
  <head>
    <meta charset="UTF-8">
    <title>Add Tournament Result</title>
    <link rel="stylesheet" href="">
     <meta name="viewport" content="width=device-width, initial-scale=1.0">
   </head>
<body>
  <div class="container">
    <div class="title">Add Tournament Result</div>
    <div class="content">
      ...
      <?php while($row = mysqli_fetch_assoc($query1)) { ?>
        <div class="user-details">
          ...
          <span class="details">Tournament</span>
            <input type="text" placeholder="Enter Result" name="Result" value=<?php echo $row['tname']; ?> readonly>
          </div>
          <div class="input-box">
            <span class="details">Match No</span>
            <input type="number" placeholder="Match No" name="matchno" required>
          </div>
          <div class="input-box">
            <span class="details">Date</span>
            <input type="date" name="date" min=<?php echo $row['sdate'];?> max=<?php echo $row['edate'];?> placeholder="Date" required>
          </div>
          <div class="input-box">
            <span class="details">Match Description</span>
            <textarea name="matchdes" placeholder="Enter Match Description" required></textarea>
          </div>
          <div class="input-box">
            <span class="details">Enter Result</span>
            <input type="text" placeholder="Enter Result" name="Result" required>
          </div>
        <div class="button"><br><br>
          <input type="submit"  name="addtourres"  value="Add Tournament Result">
          <?php } ?>
        </div>
      </form>
    </div>
